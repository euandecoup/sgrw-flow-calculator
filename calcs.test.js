...
describe("pitchAdjustmentConvertor", () => {
  test("converts pitch between 30 & 45 to 1.29 float", () => {
    expect(pitchAdjustmentConvertor(30)).toBe(1.29);
  });

  test("converts pitch between 45 & 60 to 1.5 float", () => {
    expect(pitchAdjustmentConvertor(45)).toBe(1.5);
  });

  test("converts pitch of 60 or greater to 1.87 float", () => {
    expect(pitchAdjustmentConvertor(60)).toBe(1.87);
  });

  test("returns 1 for any pitch below 30", () => {
    expect(pitchAdjustmentConvertor(0)).toBe(1);
    expect(pitchAdjustmentConvertor(15)).toBe(1);
    expect(pitchAdjustmentConvertor(29)).toBe(1);
  });
});
